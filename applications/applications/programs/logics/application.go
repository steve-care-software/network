...
import (
	...
	"steve.care/network/applications/applications/resources"
	"steve.care/network/domain/accounts"
	account_encryptors "steve.care/network/domain/accounts/encryptors"
	"steve.care/network/domain/accounts/signers"
	account_signers "steve.care/network/domain/accounts/signers"
...
func (app *application) executeInstruction(
	...
	err := app.executeInstructionResource(headFrame, resource)
	if err != nil {
		return false, nil, nil, nil, err
	}

	return false, stack, nil, currentContext, nil
}

func (app *application) executeInstructionResource(
	frame stacks.Frame,
	resource layers.InstructionResource,
) error {
	if resource.IsSave() {
		saveVariable := resource.Save()
		resource, err := frame.FetchResource(saveVariable)
		if err != nil {
			return err
		}

		return app.resourceApplication.Insert(resource)
	}

	deleteVariable := resource.Delete()
	resourceHash, err := frame.FetchHash(deleteVariable)
	if err != nil {
		return err
	}

	return app.resourceApplication.Delete(resourceHash)
}
...
func (app *application) executeAssignable(
	...
	engine := assignable.Engine()
	return app.executeEngine(
		currentAccount,
		library,
		currentContext,
		currentLayer,
...
func (app *application) executeEngine(
	currentAccount accounts.Account,
	library libraries.Library,
	currentContext receipts.Receipt,
	currentLayer layers.Layer,
	...
	retAssignable, err := app.executeAssignableResource(
		currentAccount,
		frame,
		resource,
	)
...
func (app *application) executeAssignableResource(
	currentAccount accounts.Account,
	frame stacks.Frame,
	assignableResource layers.AssignableResource,
) (stacks.Assignable, error) {
	if assignableResource.IsCompile() {
		compile := assignableResource.Compile()
		return app.executeCompile(frame, compile)
	}

	if assignableResource.IsDecompile() {
		decompile := assignableResource.Decompile()
		return app.executeDecompile(frame, decompile)
	}

	if assignableResource.IsAmountByQuery() {
		amountByQuery := assignableResource.AmountByQuery()
		return app.executeAmountByQuery(frame, amountByQuery)
	}

	if assignableResource.IsListByQuery() {
		listByQuery := assignableResource.ListByQuery()
		return app.executeListByQuery(frame, listByQuery)
	}

	if assignableResource.IsRetrieveByQuery() {
		retrieveByQuery := assignableResource.RetrieveByQuery()
		return app.executeRetrieveByQuery(frame, retrieveByQuery)
	}

	if assignableResource.IsRetrieveByHash() {
		retrieveByHash := assignableResource.RetrieveByHash()
		return app.executeRetrieveByHash(frame, retrieveByHash)
	}

	amount, err := app.resourceApplication.Amount()
	if err != nil {
		return nil, err
	}
...
func (app *application) executeCompile(
	frame stacks.Frame,
	variable string,
) (stacks.Assignable, error) {
	...
	resource, err := app.resourceApplication.Compile(data)
	if err != nil {
		return nil, err
	}
...
func (app *application) executeDecompile(
	frame stacks.Frame,
	variable string,
) (stacks.Assignable, error) {
	resource, err := frame.FetchResource(variable)
	if err != nil {
		return nil, err
	}

	data, err := app.resourceApplication.Decompile(resource)
	if err != nil {
		return nil, err
	}
...
func (app *application) executeAmountByQuery(
	frame stacks.Frame,
	variable string,
) (stacks.Assignable, error) {
	hash, err := frame.FetchHash(variable)
	if err != nil {
		return nil, err
	}

	amount, err := app.resourceApplication.AmountByQuery(hash)
	if err != nil {
		return nil, err
	}
...
func (app *application) executeListByQuery(
	frame stacks.Frame,
	variable string,
) (stacks.Assignable, error) {
	hash, err := frame.FetchHash(variable)
	...
	hashesList, err := app.resourceApplication.ListByQuery(hash)
	if err != nil {
		return nil, err
	}

	return app.assignableBuilder.Create().
		WithHashList(hashesList).
		Now()
}

func (app *application) executeRetrieveByQuery(
	frame stacks.Frame,
	variable string,
) (stacks.Assignable, error) {
	hash, err := frame.FetchHash(variable)
	...
	resource, err := app.resourceApplication.RetrieveByQuery(hash)
	if err != nil {
		return nil, err
	}
...
func (app *application) executeRetrieveByHash(
	frame stacks.Frame,
	variable string,
) (stacks.Assignable, error) {
	hash, err := frame.FetchHash(variable)
	...
	resource, err := app.resourceApplication.RetrieveByHash(hash)
	if err != nil {
		return nil, err
	}
